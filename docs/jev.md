...
The action vocabulary is fixed in `MacIntent`. Application choices map opaque request IDs to locally discovered bundle identifiers. Large app inventories are routed in bounded groups to respect the API's 255-choice limit. For a control click, enabled AX controls from a bounded observation of the selected window become candidates. If there are no usable body controls, optional ScreenCaptureKit capture and local Vision OCR provide bounded text candidates from only that window. All supported actions execute without an in-app confirmation. The driver rechecks foreground app, window identity and candidate identity before clicking; visual clicks additionally require a fresh unique label/position match and a native hit test. Screenshots never leave the process or get saved.
...
Short names can be grounded in a fresh bounded observation of the current app, including when VoiceCodex is foreground for typed entry. A unique exact observed name resolves locally; semantic goals still require a validated Jev choice. Current-app labels are excluded from literal typing and different explicitly named destinations. Observed content remains untrusted data.

OCR captions may be associated with a nearby colored tile only from actual screenshot geometry and a unique two-way label association. Both label and activation region must remain stable before input. Every click is dispatched once, then bounded local observations look for two consistent content/window transitions. Focus changes, coordinates, editable text, clock ticks, and a lone tooltip do not establish success. Unchanged or unreadable results stop the remaining sequence without a confirmation dialog or repeated click. Receipts describe observed UI changes and do not claim completion of an external business outcome.
...
- Closing a browser tab uses a separate intent from closing its entire window. The driver observes the target process/window, plus the input field or window set where relevant, and revalidates immediately before input.
